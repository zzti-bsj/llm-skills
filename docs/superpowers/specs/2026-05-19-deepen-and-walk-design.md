...
```
常规路径：理论梳理 → 理解原理 → 再去应用（慢，容易流于表面）
Deepen 路径：快速知其然 → 强制动手实现 → 做的过程中被迫理解（快，具身体验）
```

理解的欲望被真实需求驱动，而不是被阅读顺序驱动。
...
用户说 deepen jwt
  ↓
阶段 1: Quick Mastery（必经第一步）
  - 生成实战引导文档
  - 手把手带用户走完实现
  - 不要求理解，只要求动手
  - 场景可以 refresh
  ↓
阶段 2: 问题驱动深化（可选，做完 Quick Mastery 后）
  - 生成候选问题
  - 用户选择，系统写分析文档
  - 此时用户已做过一遍，理解更有基础
```

### 阶段 1: Quick Mastery（快速知其然）

#### 设计哲学

以**已经实现过这个过程的高级工程师**的视角，强制用户去执行实现。不是让用户思考——思考和理解由其他功能负责。这个功能的目标是**监督用户快速实现**。

#### 触发方式

- 用户说 `deepen <topic>` → 自动先生成 Quick Mastery 文档
- 用户说 `refresh` → 重新生成场景，保持同一技术主题

#### 文档结构

每个 Quick Mastery 文档包含三个部分：

**1. 场景（Scenario）**
一个真实的应用场景描述。不是练习题，而是「你在项目里真的会遇到的事」。

**2. 目标（Target）**
完成后的交付物——一个可运行的、能验证结果的东西。

**3. 分步引导（Steps）**
每一步包含：
- **做什么**：这一步的具体任务
- **怎么做**：实现方向和关键代码思路（不直接给完整代码，但给足够的线索）
- **为什么**：这一步背后的原理（一句话级别，不展开）

#### 生成规则

- 场景必须**具体且真实**，不能是抽象的"假设你要实现认证"
- 分步数量 3-6 步，每步可以在 10-15 分钟内完成
- 技术栈根据用户当前项目自动匹配（如果已知）
- 文件命名：`quick-mastery.md` 或 `quick-mastery【ProjectName】.md`
...
## 场景
你有一个用户管理系统（Express + PostgreSQL），所有 API 裸奔，
任何人都能调用。现在要加上 JWT 认证，只有登录用户才能访问。

## 目标
完成后：
- POST /auth/login 返回 JWT
- 其他 API 需要 Authorization header
- 未认证请求返回 401
...
### 做什么
安装 jsonwebtoken，创建一个签名密钥配置。

### 怎么做
- `npm install jsonwebtoken`
- 在 config 中定义 JWT_SECRET（不要硬编码，用环境变量）
- 设置 token 过期时间

### 为什么
密钥是 JWT 签名验证的基础。环境变量避免密钥泄露到代码仓库。
...
## 验证
用 curl 测试：未带 token → 401，带 token → 正常响应。
```
...
**主动触发**：完成 Quick Mastery 后，用户说 `deepen <topic> --questions`（或在 Quick Mastery 流程结束后系统询问是否继续）

**Walk 触发**：通过 walk 模式发现值得追问的问题后，一键转化为 deepen 任务（见下文 Walk 部分）。

**指定问题触发**：用户说 `deepen <topic> #2`，跳过问题生成，直接写指定编号的问题文档。
